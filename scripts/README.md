...
| `test-uis-pendientes.mjs` | los módulos puros de las UIs pendientes (2026-09-17): `administracion/flujos-puro` —máquinas de estado de citas y constancias, gravedades y `sanearTexto`—, `materia/actividades-puro` —el estado ACTIVA/VENCIDA **derivado** de la fecha límite, no guardado, más el reparto de pesos y el orden de presentación— el agrupado en hilos de `mensajes-internos` la constancia de estudios de Administración escolar con el **formato oficial** (`administracion/constancia-puro`: se prueba contra una constancia REAL del plantel, dato por dato —semestre en letras, «TÉCNICO EN MECATRÓNICA», fechas del semestre, fecha de expedición en letras, alumno/alumna desde la CURP—) y el número de control (`alumno/numero-control-puro`, la misma regla que el CHECK de la base). **92 verificaciones.** Las que más valen son las que prohíben un salto: una cita `pendiente` no puede pasar a `finalizada` sin aceptarse, y una constancia no se entrega sin aprobarse |
