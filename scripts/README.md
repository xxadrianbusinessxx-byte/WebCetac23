...
| `test-uis-pendientes.mjs` | los módulos puros de las UIs pendientes (2026-09-17): `administracion/flujos-puro` —máquinas de estado de citas y constancias, gravedades y `sanearTexto`—, `materia/actividades-puro` —el estado ACTIVA/VENCIDA **derivado** de la fecha límite, no guardado, más el reparto de pesos y el orden de presentación— el agrupado en hilos de `mensajes-internos` y la constancia de estudios **beta** de Administración escolar (`administracion/constancia-puro`: arma el texto con el expediente y declara pendientes folio, CCT, firma y sello en vez de inventarlos). **73 verificaciones.** Las que más valen son las que prohíben un salto: una cita `pendiente` no puede pasar a `finalizada` sin aceptarse, y una constancia no se entrega sin aprobarse |
