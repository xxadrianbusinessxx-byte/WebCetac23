...
| `gen-panel-repo.mjs` | (`LEE(fs)`) genera `docs/sistema/panel-repo.html`: el estado del repo de un vistazo — las 10 reglas de ORDEN con sus umbrales, el nº de suites, el **coste de arranque en tokens** de la lectura obligatoria, y el avance del restyle. **No mide nada por su cuenta**: ejecuta `test-orden.mjs --json` y `diag-restyle-oceano.mjs --json` y junta lo que devuelven, porque medir aquí daría dos fuentes para la misma verdad (R6). Existe porque el repo medía mucho y no enseñaba nada: con cinco comandos y cinco salidas de texto, en la práctica no se mira — y los archivos de >1 000 líneas pasaron de 4 a 7 sin que nadie lo notara |
