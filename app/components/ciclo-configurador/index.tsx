...
const input = "rounded-lg border bg-[var(--oc-input)] px-2 py-1.5 text-xs font-semibold text-[var(--oc-muted)]";
const btn = "rounded-full bg-[var(--oc-surface)] px-4 py-2 text-[10px] font-extrabold uppercase text-[var(--oc-text)] disabled:opacity-50";
...
export function CicloConfigurador({ periodoIdInicial }: { periodoIdInicial?: string } = {}) {
  ...
  function renderEliminarOverlay() {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--oc-surface)] p-4" role="dialog" aria-modal="true">
        <div className="w-full max-w-md rounded-2xl border border-[var(--oc-border)] bg-[var(--oc-input)] p-4 text-[var(--oc-muted)] shadow-2xl">
          <p className="text-[10px] font-extrabold uppercase tracking-wide text-[var(--oc-alert-text)]">
            Eliminar ciclo completo
          </p>
          <p className="mt-1 text-sm font-extrabold text-[var(--oc-muted)]">{eliminando.nombre}</p>
          {eliminando.cargandoDiagnostico ? (
            <p className="mt-2 text-xs font-semibold text-[var(--oc-muted)]">Calculando conteos…</p>
          ) : !d ? (
            <p className="mt-2 text-xs font-semibold text-[var(--oc-muted)]">
              {eliminando.error ?? "Sin diagnóstico."}
            </p>
          ) : !d.ok ? (
            <p className="mt-2 rounded-xl bg-[var(--oc-alert)]/15 px-3 py-2 text-xs font-semibold text-[var(--oc-alert-text)]">{d.error}</p>
          ) : (
            <>
              <div className="mt-2 grid grid-cols-2 gap-1 rounded-xl bg-[var(--oc-surface)] p-2 text-[10px] font-semibold text-[var(--oc-muted)] sm:grid-cols-3">
                <span>Grupos: {d.conteos.grupos}</span>
                <span>Grupo-materias: {d.conteos.grupoMaterias}</span>
                <span className={d.conteos.inscripciones > 0 ? "text-[var(--oc-alert-text)]" : ""}>Inscripciones: {d.conteos.inscripciones}</span>
              ...
              {d.bloqueos.length > 0 && (
                <ul className="mt-2 space-y-1 rounded-xl bg-[var(--oc-alert)]/15 p-2 text-[10px] font-bold text-[var(--oc-alert-text)]">
                  {d.bloqueos.map((b) => <li key={b}>· {b}</li>)}
                </ul>
              )}
              {d.bloqueos.length === 0 && (
                <>
                  <p className="mt-2 text-[10px] font-bold uppercase text-[var(--oc-muted)]">Confirmación</p>
                  <input
              ...
              {eliminando.error && (
                <p className="mt-2 rounded-xl bg-[var(--oc-alert)]/15 px-3 py-2 text-xs font-semibold text-[var(--oc-alert-text)]">{eliminando.error}</p>
              )}
            </>
          )}
          <div className="mt-3 flex flex-wrap justify-end gap-2">
            <button type="button" className="rounded-full border border-[var(--oc-border)] bg-[var(--oc-input)] px-4 py-1.5 text-[10px] font-extrabold uppercase tracking-wide text-[var(--oc-muted)]" onClick={cerrarEliminar} disabled={eliminando.trabajando}>Cancelar</button>
            {d && d.ok && d.bloqueos.length === 0 && (
              <button
                type="button"
                disabled={eliminando.confirmacion.trim().toUpperCase() !== d.nombre || eliminando.trabajando}
                className="rounded-full bg-[var(--oc-alert)]/15 px-4 py-1.5 text-[10px] font-extrabold uppercase tracking-wide text-[var(--oc-text)] disabled:opacity-50"
  ...
  return (
    <section className="relative mt-6 rounded-2xl border border-[var(--oc-border)] bg-[var(--oc-surface)] p-3 sm:p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-sm font-extrabold uppercase text-[var(--oc-text)]">Configuración del ciclo</h2>
        {sel && <span className={`rounded-full px-2 py-0.5 text-[9px] font-extrabold uppercase ${sel.activo ? "bg-[var(--oc-ok)]/15 text-[var(--oc-ok)]" : "bg-[var(--oc-alert)]/15 text-[var(--oc-alert-text)]"}`}>{sel.nombre} · {sel.activo ? "OPERATIVO" : "BORRADOR"}</span>}
      </div>
      {msg && <p className={`mt-2 rounded-xl px-3 py-2 text-xs font-bold ${msg.t === "ok" ? "bg-[var(--oc-ok)]/15 text-[var(--oc-ok)]" : "bg-[var(--oc-alert)]/15 text-[var(--oc-alert-text)]"}`}>{msg.x}</p>}
      <div className="mt-3 flex flex-col gap-2">
        {ciclos.length === 0 ? (
          <p className="text-[10px] font-bold uppercase text-[var(--oc-muted)]">Sin ciclos todavía. Crea el primero abajo.</p>
        ) : (
          <div className="flex max-h-44 flex-col gap-1 overflow-y-auto pr-1">
            {ciclos.map((c) => {
              const esSeleccionado = c.id === periodoId;
              return (
                <div key={c.id} className={`flex items-center justify-between gap-2 rounded-lg border px-2 py-1 text-[10px] font-extrabold uppercase transition ${esSeleccionado ? "border-[var(--oc-border)] bg-[var(--oc-surface)] text-[var(--oc-text)]" : "border-[var(--oc-border)] bg-[var(--oc-input)] text-[var(--oc-muted)]"}`}>
                  <button type="button" className="min-w-0 flex-1 truncate text-left" title={`Seleccionar ${c.nombre}`} onClick={() => void seleccionar(c.id)}>
                    {c.nombre}
                    <span className={`ml-2 inline-block rounded-full px-2 py-0.5 text-[8px] font-extrabold ${c.activo ? "bg-[var(--oc-ok)]/15 text-[var(--oc-ok)]" : "bg-[var(--oc-surface)] text-[var(--oc-muted)]"}`}>{c.activo ? "OPERATIVO" : (c.estado ?? "BORRADOR")}</span>
                  </button>
                  <div className="flex shrink-0 items-center gap-1">
                    <button type="button" disabled={c.activo} title={c.activo ? "Ya es el ciclo operativo actual" : "Activar como OPERATIVO (misma validación del Paso 7)"} className="rounded-full bg-[var(--oc-ok)]/15 px-3 py-1 text-[9px] font-extrabold uppercase tracking-wide text-[var(--oc-text)] transition hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-40" onClick={() => void activarDesdeLista(c.id)}>
                      {c.activo ? "Operativo" : "Activar"}
                    </button>
                    {!c.activo && (
                      <button type="button" title="Eliminar ciclo completo (doble confirmación)" className="rounded-full bg-[var(--oc-alert)]/15 px-3 py-1 text-[9px] font-extrabold uppercase tracking-wide text-[var(--oc-text)] transition hover:brightness-110" onClick={() => void abrirEliminar(c.id, c.nombre)}>
                        Eliminar
                      </button>
                    )}
                  </div>
      ...
      <div className="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-[11rem_1fr]">
        <nav className="flex flex-wrap gap-1 lg:flex-col">
          {PASOS.map((p) => (
            <button key={p.id} type="button" className={`rounded-lg px-2 py-1.5 text-left text-[10px] font-extrabold uppercase ${paso === p.id ? "bg-[var(--oc-surface)] text-[var(--oc-text)]" : "bg-[var(--oc-input)] text-[var(--oc-text)]"}`} onClick={() => setPaso(p.id)}>{p.label}</button>
          ))}
        </nav>
        <div className="min-h-[24rem] rounded-2xl border border-[var(--oc-border)] bg-[var(--oc-input)] p-3">
          {!periodoId ? <p className="text-xs font-semibold text-[var(--oc-muted)]">Selecciona o crea un ciclo.</p>
