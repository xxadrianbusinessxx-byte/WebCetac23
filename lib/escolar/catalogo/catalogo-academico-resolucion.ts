 ...
 *
 * Las filas del catálogo (`periodos`, `carreras`, `materias`, `grupos`,
 * `grupo_materias`, `inscripciones_alumno`, `asignaciones_profesor`), la
 * normalización G2 de texto y la resolución de identidad: inscripción activa
 * del alumno, identidad de una materia y oferta grupo→materia.
 *
 * Es la parte mayor del antiguo `lib/escolar/catalogo/catalogo-academico.ts`
 * (PROMPT E · R-3). La administración (acceso y atribución de profesor,
 * visibilidad de materias) se quedó en `./catalogo-academico.ts`, que
 * re-exporta esta parte para que ningún import existente se rompa.
 */
...
export function normClave(clave: string): string {
