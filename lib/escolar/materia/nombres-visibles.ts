 ...
 * La fila y su historial se conservan (R8), y volver a ponerlo es un clic
 * (guardarNombreVisibleMateria vuelve a activarlo). Al quedar inactivo, la
 * materia vuelve a mostrarse por su idInterno en toda la UI.
 */
export async function quitarNombreVisibleMateria(
  supabase: SupabaseClient,
  idInterno: string,
): Promise<{ ok: true } | { ok: false; error: string }> {
  const id = idInterno.trim();
  ...
  const { data: fila, error: eSel } = await supabase
    .from(TABLA_NOMBRES_VISIBLES)
    .select("materia_id, activo")
    .eq("materia_id", id)
    .maybeSingle();
  if (eSel) return { ok: false, error: eSel.message };

  if (!fila || fila.activo === false) {
    return { ok: true }; // ya sin alias activo: idempotente
  }
