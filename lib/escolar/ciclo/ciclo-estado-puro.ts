...
/* ---------------------------------------------------------------------------
 * PROMPT-4/T1 (opción A) — Sincronización de inscripciones respetando la
 * decisión humana. Lógica PURA (sin Supabase) para poder probarse.
 *
 * `activo` significa «pertenece al ciclo operativo». `decision_manual = true`
 * significa «un humano decidió explícitamente sobre esta fila» (p. ej. la
 * deduplicación del PROMPT-1/T3: este alumno no va en este grupo). La
 * sincronización elige la fila más reciente por CURP entre las NO marcadas, y
 * nunca toca (activa ni desactiva) una fila marcada.
 * ------------------------------------------------------------------------- */

export type FilaInscripcionSync = {
  id: string;
  curp: string;
  grupo_id: string;
  activo: boolean;
  /** PROMPT-4/T1: true = decisión humana; la sincronización no la toca. */
  decision_manual?: boolean | null;
  created_at?: string | null;
};

export type PlanSincronizacion = {
  /** Filas que pasarían a activo=true. */
  activarIds: string[];
  /** Filas que pasarían a activo=false. */
  desactivarIds: string[];
  /** Filas marcadas que quedan intactas (traza). */
  intactasIds: string[];
};

export const marcada = (f: { decision_manual?: boolean | null }): boolean =>
  f.decision_manual === true;

/**
 * Decide, para un conjunto de filas DENTRO de un mismo ciclo, cuáles activar,
 * cuáles desactivar y cuáles no tocar. Mismo orden que la versión original:
 * por CURP, la más reciente por created_at (desc) y, a igualdad, por id (desc).
 * PROMPT-4/T1: las marcadas con decision_manual=true se excluyen de la elección
 * y jamás se listan en activarIds ni desactivarIds.
 */
export function planSincronizacionInscripciones(
  filas: readonly FilaInscripcionSync[],
): PlanSincronizacion {
  const porCurp = new Map<string, FilaInscripcionSync[]>();
  ...
  const intactasIds: string[] = [];

  for (const lista of porCurp.values()) {
    const marcadasLocal = lista.filter(marcada);
    for (const m of marcadasLocal) intactasIds.push(m.id);

    const candidatas = lista.filter((f) => !marcada(f));
    ...
    for (const f of candidatas) {
      if (f.id === elegida?.id) {
        if (!f.activo) activarIds.push(f.id);
      } else if (f.activo) {
        desactivarIds.push(f.id);
      }
    }
  }

  return { activarIds, desactivarIds, intactasIds };
}

/**
 * Simula la aplicación del plan sobre las filas y devuelve el estado final de
 * `activo` (para pruebas y para la medición antes/después). Las marcadas se
 * quedan exactamente como estaban.
 */
export function aplicarPlanSincronizacion(
  filas: readonly FilaInscripcionSync[],
  plan: PlanSincronizacion,
): FilaInscripcionSync[] {
  const activar = new Set(plan.activarIds);
  const desactivar = new Set(plan.desactivarIds);
  return filas.map((f) => {
    if (marcada(f)) return f;
    if (activar.has(f.id)) return { ...f, activo: true };
    if (desactivar.has(f.id)) return { ...f, activo: false };
    return f;
  });
}
