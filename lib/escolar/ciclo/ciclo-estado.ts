 ...
 *
 * PROMPT-4/T1 (opción A): las filas con `decision_manual = true` son una
 * decisión humana explícita (p. ej. «este alumno no va en este grupo», puesta
 * por el roster/deduplicación). La sincronización NO las toca: ni las activa,
 * ni las desactiva, ni las elige como «la más reciente». El humano decide; la
 * fecha decide solo entre lo que el humano no marcó.
 */
async function sincronizarInscripcionesOperativo(
  ...
  // PROMPT-4/T1 (opción A): si la columna aditiva aún no está aplicada,
  // decision_manual es undefined → se comporta como false (nada marcado).
  // Compatibilidad aditiva: primero se intenta con la columna; si no existe
  // (42703) se reintenta sin ella (comportamiento anterior a PROMPT-4).
  const leerFilas = async (conColumna: boolean) => {
    const cols = conColumna
      ? "id, curp, grupo_id, activo, decision_manual, created_at"
      : "id, curp, grupo_id, activo, created_at";
    return supabase
      .from(TABLA_INSCRIPCIONES_ALUMNO)
      .select(cols)
      .in("grupo_id", grupoIds);
  };
  let resFilas = await leerFilas(true);
  if (
    resFilas.error &&
    /42703|does not exist/i.test(String(resFilas.error.message ?? ""))
  ) {
    resFilas = await leerFilas(false);
  }
  if (resFilas.error) return resFilas.error.message;
  const filas = (
    ((resFilas.data ?? []) as unknown as Array<Record<string, unknown>>).map(
      (f) => ({
        ...f,
        decision_manual: (f.decision_manual ?? false) as boolean,
      }),
    ) as unknown as FilaInscripcionSync[]
  );

  // La DECISIÓN (qué activar/desactivar y qué NO tocar) vive en el módulo puro.
  const plan = planSincronizacionInscripciones(filas);

  // Filas activas de OTROS ciclos pasan a inactivas (ya no hay otro operativo).
  // PROMPT-4/T1: las marcadas con decision_manual tampoco se tocan aquí.
  ...
  if (!eO && (otrosObj ?? []).length > 0) {
    ...
    const { data: activasOtros, error: eAO } = await supabase
      .from(TABLA_INSCRIPCIONES_ALUMNO)
      .select("id, decision_manual")
      .in("grupo_id", otrosIds)
      .eq("activo", true);
    if (eAO) return eAO.message;
    for (const r of (activasOtros ?? []) as Array<{ id: string; decision_manual?: boolean | null }>) {
      if (r.decision_manual === true) continue;
      plan.desactivarIds.push(porId(r.id));
    }
  }
  ...
  const err1 = await aplicar(plan.desactivarIds, false);
  if (err1) return err1;
  return aplicar(plan.activarIds, true);
}
