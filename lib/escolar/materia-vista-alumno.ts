...
function filaPareceAlumno(
  ...
  const colNombre = colsDatos.includes("alumno_nombre")
    ? "alumno_nombre"
    : colsDatos[0];
  if (!colNombre) return false;
  const n = String(row[colNombre] ?? "").trim();
...
function coincideNombreEnFila(
  row: Record<string, unknown>,
  colsDatos: string[],
  criterio: CriterioAlumnoEnFila,
): boolean {
  const curpU = criterio.curp?.trim().toUpperCase();
  if (curpU) {
    for (const col of colsDatos) {
      if (String(row[col] ?? "").trim().toUpperCase() === curpU) return true;
    }
  }
  ...
  if (colNombre) {
    ...
    if (
      ...
      celdaContieneNombreCriterio(enColumna, criterio)
    ) {
      return true;
    }
  }
  ...
  if (esFilaEncabezadoMateria(celdas) || esFilaMetadatoPlantel(celdas)) {
    return false;
  }
  return filaCoincideAlumno(celdas, criterio);
}

function celdaContieneNombreCriterio(
  texto: string,
  criterio: CriterioAlumnoEnFila,
): boolean {
  const base = criterio.nombreCompleto?.trim() ?? "";
  const alt = criterio.nombresAlternativos ?? [];
  if (base && nombresMismoAlumno(texto, base)) return true;
  for (const v of alt) {
    if (nombresMismoAlumno(texto, v)) return true;
  }
  return false;
}

function buscarIndiceFilaAlumno(
  rows: Record<string, unknown>[],
  colsDatos: string[],
  criterio: CriterioAlumnoEnFila,
): number {
  for (let i = 0; i < rows.length; i++) {
    if (coincideNombreEnFila(rows[i]!, colsDatos, criterio)) return i;
  }
  return -1;
}
...
export async function leerVistaMateriaAlumno(
  ...
  if (idx < 0 && token) {
    ...
    if (!errFull && todasDb?.length) {
      todas = todasDb as Record<string, unknown>[];
      idx = buscarIndiceFilaAlumno(todas, colsDatos, criterio);
    }
  }
